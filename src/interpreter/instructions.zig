...
pub const TABLE: [256]InstructionPtr = init: {
    ...
    for (0..256) |i| {
        map[i] = opcodeToFnImpl(@enumFromInt(i));
    }
    break :init map;
};

/// Returns the function pointer for the given opcode.
pub inline fn opcodeToFn(opcode: Opcode) InstructionPtr {
    return TABLE[@intFromEnum(opcode)];
}

fn opcodeToFnImpl(opcode: Opcode) InstructionPtr {
